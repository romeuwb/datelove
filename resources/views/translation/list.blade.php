...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-200">
        <?= __tr('__languageName__ Language Translations (__translationsCount__)', [
    ...
    <div class="btn-group" role="group">
        <a class="btn btn-light" href="<?= route('manage.translations.languages') ?>"
            title="<?= __tr('Back to languages') ?>"><i class="fa fa-arrow-left"></i>
            <?= __tr('Back to languages') ?>
        </a>
        <a class="btn btn-secondary lw-ajax-link-action" href="<?= route('manage.translations.scan', [
         'languageId' => $languageInfo['id'],
     ]) ?>" title="<?= __tr('Re-Scan') ?>"><i class="fa fa-sync-alt"></i>
            <?= __tr('Re-Scan') ?>
        </a>
        ...
        <div class="btn-group" role="group">
            ...
            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="btnGroupDrop1">
                ...
                <!-- Other services -->
                @if (getStoreSettings("microsoft_translator_api_key")) <a class="dropdown-item lw-ajax-link-action"
                    data-method="post" href="<?= route('manage.translations.auto_translate', [
'serviceId' => 'microsoft',
'languageId' => $languageInfo['id'],
]) ?>" title="<?= __tr('Re-Scan') ?>"><i class="fa fa-sync-alt"></i> {{ __tr('Microsoft Translator') }}</a>
                @else
                <a href="#" disabled class="disabled dropdown-item"> {{ __tr('Microsoft Translator - Key not
                    added') }}</a>
                @endif
                <a class="dropdown-item" href="#autoTranslationDialog" data-toggle="modal"
                    data-target="#autoTranslationDialog">
...
<!-- Modal -->
<div class="modal fade" id="autoTranslationDialog" tabindex="-1" role="dialog" aria-labelledby="autoTranslationLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="autoTranslationLabel">
                    <?= __tr('Auto translations using Google Spreadsheet') ?>
                </h5>
            ...
            <div class="modal-body">
                ...
                <div>
                    ...
                    <!-- Export button -->
                    <a class="btn btn-secondary btn-sm mt-2" target="_blank"
                        href="<?= route('manage.translations.export', ['languageId' => $languageInfo['id']]) ?>"
                        title="<?= __tr('Export') ?>"><i class="fas fa-file-export"></i>
                        <?= __tr('Export Translation Strings to XLSX file') ?>
                    </a>
                    <!-- /Export button -->
                </div>
                <!-- /Step 1 -->
                ...
                <div class="col-lg-9 mt-2">
                    <input type="file" name="filepond" class="filepond lw-file-uploader mt-5" id="lwFileUploader"
                        data-remove-media="true" data-instant-upload="true"
                        data-allowed-media='<?= getMediaRestriction('language') ?>' data-action="<?= route('manage.translations.import', ['languageId' => $languageInfo['id']]) ?>"
                    data-label-idle="<span class='filepond--label-action'>
                        <?= __tr('Import & Process') ?>
                    </span>" data-callback="afterSuccessfullyUploaded">
                </div>
            </div>
...
<div class="row">
    <div class="col-xl-12 mb-4 <?= $languageInfo['is_rtl'] ? 'lw-lang-direction-rtl' : 'lw-lang-direction-ltr' ?>">
        <div class="alert alert-warning">
            ...
            <?= __tr('Google Auto Translate given here is API key less method your IP may get BLOCKED for particular time, if too much requests are done.') ?>
        </div>
        <?php $lineCount = 1; ?>
                ...
                <form class="row lw-ajax-form lw-form" method="post" action="<?= route('manage.translations.update') ?>"
                    ...
                    <div class="input-group mb-3">
                        ...
                        <div class="input-group-append">
                            <button class="btn btn-outline-secondary lw-auto-translate-action" type="button"
                                title="<?= __tr('Google Auto Translate') ?>"><i class="fa fa-language"></i>
                                <?= __tr('Auto Translate') ?>
                            </button>
                            <button class="btn btn-light lw-save-translation lw-ajax-form-submit-action" type="button">
                                <?= __tr('Save') ?>
                            </button>
                        </div>
                    </div>
