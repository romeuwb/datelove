...
class TranslationEngine extends BaseEngine implements TranslationEngineInterface
{
    ...
    public function scan($languageId)
    {
        ...
        // Meta for PoEdit
        // $headers->set('X-Poedit-Basepath', "../../..\n");
        $headers->set('X-Poedit-KeywordsList', "__;__tr;__trn\n");
        // $headers->set('X-Poedit-SearchPath-0', "app\n");
        // $headers->set('X-Poedit-SearchPath-1', "resources\n");
        // $headers->set('X-Poedit-SearchPath-2', "config\n");
    ...
    public function update($inputData)
    {
        ...
        if (!\file_exists($existingPoFile)) {
            $this->scan($languageId);
            // return $this->engineResponse(2, null, __tr('Language PO file not found'));
        }
    ...
    */
        ...
        if (!empty($translations)) {
            ...
            try {
                ...
                switch ($serviceId) {
                    ...
                    case 'microsoft':
                        $translationServiceResult = $this->microsoftTranslate($languageId, $exportTranslations);
                        break;
    ...
    protected function microsoftTranslate(string $languageId, array $data): array
    {
        ...
        $languageId = substr($languageId, 0, 2);
        // get the key
        $key = getStoreSettings("microsoft_translator_api_key");;
        // check if it exists
        if (!$key) {
        ...
        $resultDecoded = [];
        // Microsoft has limit of 100 translation per request
        foreach (array_chunk($data, 80) as $dataItem) {
