...
/**
 * Core Helper - 1.12.36 - 06 OCT 2023
 *
...
if (config('app.debug') == false) {
    if ($debugIps = config('__tech.app_debug_ips', false)) {
        if ($debugIps) {
...
if (! function_exists('__response')) {
    function __response($data, $reactionCode = 1)
    {
        ...
        if (config('app.debug', false) == true) {
            ...
            if (config('__tech.mail_view_debug', false) == true) {
...
if (! function_exists('__yesset')) {
    ...
    function __yesset($file, $generateTag = false, $options = [])
    {
        ...
        foreach ($files as $keyFile) {
            ...
            if (empty($globFiles)) {
                __dd(glob($keyFile, GLOB_BRACE), $keyFile);
